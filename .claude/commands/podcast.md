...
- ⏳ **P2**: 外部リンク fetch (未実装、次の commit で追加)
- ⏳ **P3**: 関連ニュース取得 (未実装)
- ⏳ **P4**: 脚本生成 (未実装、ここまでで `--dry-run` 完成予定)
- ⏳ **P5**: TTS 合成 (未実装)
- ⏳ **P6**: ffmpeg mix (未実装)

P1 までの時点では `/podcast` を呼ぶと「期間内のいいね N 件 / ペルソナ候補 M 案」
を提示してユーザーに承認を取った後、「P2 以降が未実装なのでここまで」と
返して終了する。フル機能は P4 完了後に解禁。
...
### Stage 2: 外部リンク fetch (P2 で実装)

**現状未実装。P2 で以下を追加予定:**

```bash
pnpm tsx src/scripts/podcast/extract-links.ts < /tmp/podcast-tweets.json > /tmp/podcast-urls.json
```

その後、メインセッションが WebFetch で 1 件ずつ要約 (200 字以内) し
`data/podcast-link-cache.json` に永続化。

### Stage 3: 関連ニュース取得 (P3 で実装)

**現状未実装。P3 で以下を追加予定:**

カテゴリ上位 2 つそれぞれに対して WebSearch で
「{category-label} 2026年X月 話題」を検索、上位 1-2 件を news_context として
`/tmp/podcast-news.json` に保存。
...
2. Stage 1 を実行、件数を確認
3. Stage 4 を実行、AskUserQuestion でペルソナ承認
4. ユーザーに「P2 以降は未実装。期間 {from}〜{to} / N 件 / ペルソナ確定: {hosts}」を報告して終了
