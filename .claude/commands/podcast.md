...
- ⏳ **P3**: 関連ニュース取得 (未実装、次の commit で追加)
- ⏳ **P4**: 脚本生成 (未実装、ここまでで `--dry-run` 完成予定)
- ⏳ **P5**: TTS 合成 (未実装)
- ⏳ **P6**: ffmpeg mix (未実装)

P2 までの時点では `/podcast` を呼ぶと Stage 1-2 を実行 → ペルソナ承認 →
「P3 以降が未実装なのでここまで」と返して終了する。フル機能は P4 完了後に解禁。
...
4. Stage 4 を実行、AskUserQuestion でペルソナ承認
5. ユーザーに「P3 以降は未実装。期間 {from}〜{to} / N 件 / 外部リンク fetched=X / ペルソナ確定: {hosts}」を報告して終了
