...
**2025 年分以降は月号運用** (TTS コスト削減のため): `--from YYYY-MM-01 --to YYYY-MM-末日`
で暦の 1 ヶ月を 1 エピソードにまとめる (例 1 月号: `--from 2025-01-01 --to 2025-01-31`)。
slug は `2025-01-01_to_2025-01-31` となり、タイトル/章立て/尺は Stage 9 の月号ルールに従う。
2024 年分までは週次 (約 7 日) のまま。
...
タイトルは期間の形で統一する:
- **週次** (約7日、2024年分まで): 「いいねダイジェスト {from}週 第${EP_NO}回 (上位2カテゴリ)」
- **月号** (暦の1ヶ月 = `--from YYYY-MM-01 --to YYYY-MM-末日`、**2025年分以降はこちら**):
  「いいねダイジェスト {YYYY}年{M}月号 第${EP_NO}回 (上位2カテゴリ)」(例「いいねダイジェスト 2025年1月号 第9回」)。
  月号は週次の約4倍のツイートを含むので、TTS コストを抑えるため尺は据え置き
  (6,000-8,000 字) で、章立てはテーマで束ねて代表ツイートを厳選する。
